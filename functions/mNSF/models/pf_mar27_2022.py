...
class ProcessFactorization(tf.Module):
  ...
  def sample_predictive_mean(self, X, W__, sz=1, S=1, kernel=None, mu_z=None, Kuu_chol=None, chol=True):
    """
    ...
    if self.nonneg:
      Lam = tfl.matrix_transpose(tfl.matmul(W__, tf.exp(F))) #SxNxJ
      if self.lik=="gau":
        return Lam
      else:
        return sz*Lam
    else:
      Lam = tfl.matrix_transpose(tfl.matmul(W__, F))
