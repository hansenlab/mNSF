...
# NOTE - errors at line 201 fits[i].sample_latent_GP_funcs(list_D__[i]['X'],S=100,chol=True) with error InvalidArgumentError: {{function_node __wrapped__Sub_device_/job:localhost/replica:0/task:0/device:GPU:0}} required broadcastable shapes [Op:Sub]
...
samples = [D1, D2, D3, D4, D1, D2, D3, D4, D1, D2, D3, D4] # include all samples for analysis in list
sample_names = ['D1', 'D2', 'D3', 'D4', 'D1', 'D2', 'D3', 'D4', 'D1', 'D2', 'D3', 'D4']
nsamples_toIterate = list(range(2, 13))
#nsample = len(samples)

run_ID = "KW_mouseSagittal_4samples_singleScript_generalized"
...
output_memory = pd.DataFrame(np.zeros((len(nsamples_toIterate), 5)))
output_memory.columns = ["nSamples", "peak_before_first_iteration", "peak_after_first_iteration", "peak_after_all_iterations", "runtime"]
output_memory_file = "memoryUsage_mouseSagittal_niter50_L50_iterateNSamples.csv"


for nsample in nsamples_toIterate:
    samples = samples[:nsample]
    sample_names = sample_names[:nsample]
    ...
    output_memory.iloc[i, 0] = nsample
    start = time.time()
    output_memory.iloc[i, 1] = tf.config.experimental.get_memory_info("GPU:0")['peak']/(1024*1024*1024)
    print('Before creating tro', tf.config.experimental.get_memory_info("GPU:0"))
