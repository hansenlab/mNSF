...
for ksample in range(0,12):
	print(ksample)
	##### load data
	Y=pd.read_csv('Y_alllGenes_2percentSparseFilter_sample'+str(ksample+1)+'.csv')
	X=pd.read_csv('X_allSpots_sample'+str(ksample+1)+'.csv')
	##### process data
	...
	DF.to_csv(path.join('dev_poisson_sample'+str(ksample+1)+'.csv'))
