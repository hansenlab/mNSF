...
def evaluate_induced_points_percentage(list_D, L, nsample, percentages=[0.05, 0.1, 0.15, 0.2, 0.3]):
    ...
    for pct in percentages:
        ...
        for ksample in range(nsample):
            ...
            # Add 'sz' key if missing (using default value of 1.0 for all spots)
            if 'sz' not in D_copy:
                D_copy['sz'] = np.ones(n_spots)
        ...
        try:
            ...
            print(f"Error with {pct*100}% induced points: {str(e)}")
            # Add a record with error information
            results.append({
                'Percentage': pct * 100,
                'Induced_Points': n_induced,
                'Deviance': float('nan'),  # NaN for error cases
    ...
    # Visualize (only for successful runs)
    valid_results = results_df[~results_df['Deviance'].isna()].copy()
    
    if len(valid_results) > 0:
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))
        
        # Plot deviance
        ax1.plot(valid_results['Percentage'], valid_results['Deviance'], 'b-o')
        ...
        # Plot computation time
        ax2.plot(valid_results['Percentage'], valid_results['Time'], 'r-s')
        ax2.set_xlabel('Induced Points (%)')
        ax2.set_ylabel('Computation Time (seconds)')
...
def compare_induced_points_strategies(list_D, L, nsample, percentage=0.15):
    ...
    for ksample in range(nsample):
        ...
        # Check if 'sz' key exists, add if missing
        if 'sz' not in list_D[ksample]:
            list_D[ksample]['sz'] = np.ones(n_spots)
    ...
    # Calculate average deviance for each strategy (excluding errors)
    valid_results = results_df[~results_df['Deviance'].isna()]
    if len(valid_results) > 0:
        strategy_summary = valid_results.groupby('Strategy')['Deviance'].agg(['mean', 'std']).reset_index()
        
        # Visualize
        plt.figure(figsize=(10, 6))
        sns.barplot(data=valid_results, x='Strategy', y='Deviance')
        plt.title('Performance Comparison of Induced Points Selection Strategies')
        plt.xlabel('Selection Strategy')
        plt.ylabel('Poisson Deviance (lower is better)')
        plt.grid(axis='y', alpha=0.3)
        plt.tight_layout()
        plt.show()
    else:
...
### Example 1: Evaluating Different Percentages of Induced Points
...
# Load example spatial transcriptomics data (assuming data is already preprocessed)
# This would typically be loaded from a file
data_path = "path/to/your/data/"
list_D = []  # List to hold data dictionaries for each sample

# For this example, we'll assume we have 3 samples
for sample_id in range(1, 4):
    # Load data for this sample (in practice, replace with actual data loading)
    sample_file = f"{data_path}sample_{sample_id}.h5ad"
    print(f"Loading data from {sample_file}...")
    
    # In a real example, you would load actual data here
    # For demonstration purposes, we'll create synthetic data
    n_spots = 2000 + sample_id * 500  # Different number of spots per sample
    n_genes = 100
    
    # Create synthetic spatial coordinates
    X = np.random.rand(n_spots, 2) * 100
    
    # Create synthetic gene expression data
    Y = np.random.negative_binomial(5, 0.5, size=(n_genes, n_spots))
...
# Set parameters
L = 8  # Number of factors
nsample = len(list_D)  # Number of samples

# Example usage of evaluate_induced_points_percentage
print("Running evaluation of different induced points percentages...")
for ksample in range(len(list_D)):
    n_spots = list_D[ksample]['X'].shape[0]
    list_D[ksample]['sz'] = np.ones(n_spots)  # Default size factor of 1.0 for all spots

# Now try running the function again
results_df = evaluate_induced_points_percentage(
    list_D, 
    L=L, 
    nsample=len(list_D), 
    percentages=[0.15]  # Start with just one percentage to test
)

# Print the results
print("\nResults from induced points percentage evaluation:")
print(results_df)

# Find the optimal percentage based on the balance of deviance and computation time
# We'll use a simple scoring function that weights both metrics
results_df['deviance_norm'] = (results_df['Deviance'] - results_df['Deviance'].min()) / (results_df['Deviance'].max() - results_df['Deviance'].min())
results_df['time_norm'] = (results_df['Time'] - results_df['Time'].min()) / (results_df['Time'].max() - results_df['Time'].min())
results_df['score'] = 0.7 * results_df['deviance_norm'] + 0.3 * results_df['time_norm']

optimal_pct = results_df.loc[results_df['score'].idxmin(), 'Percentage']
print(f"\nOptimal percentage of induced points: {optimal_pct}%")
```

### Example 2: Comparing Different Induced Points Selection Strategies

```python
# Using the same data from Example 1
# Compare different strategies for selecting induced points

print("\nComparing different induced points selection strategies...")
strategy_results = compare_induced_points_strategies(
    list_D, 
    L=L, 
    nsample=nsample, 
    percentage=0.15  # Using 15% of points as induced points
)
...
# Calculate average deviance for each strategy
strategy_summary = results_df.groupby('Strategy')['Deviance'].agg(['mean', 'std']).reset_index()
print("\nAverage deviance by strategy:")
print(strategy_summary)

# Visualize results
plt.figure(figsize=(10, 6))
sns.barplot(x='Strategy', y='Deviance', data=results_df)
plt.title('Model Performance by Induced Points Selection Strategy')
plt.ylabel('Poisson Deviance (lower is better)')
plt.grid(axis='y', alpha=0.3)
plt.tight_layout()
plt.show()
...
best_strategy = strategy_summary.loc[strategy_summary['mean'].idxmin(), 'Strategy']
print(f"\nBest induced points selection strategy: {best_strategy}")
```
