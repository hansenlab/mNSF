...
class ProcessFactorization_self12(tf.Module):
  def __init__(self, self1_0, self2_0, J, L, Z1, Z2, lik="poi", psd_kernel=tfk.MaternThreeHalves, 
    ...
    self._init_misc()###>>>
    #rep1
    #self.Kuu_chol1 = tf.Variable(self.eval_Kuu_chol_rep1(self.get_kernel()), dtype=dtp, trainable=False)
    #rep1
    #self.Kuu_chol2 = tf.Variable(self.eval_Kuu_chol_rep2(self.get_kernel()), dtype=dtp, trainable=False)
    if self.lik=="gau" and not self.nonneg:
      self.feature_means = feature_means
    else:
      self.feature_means = None
  ...
  def elbo_avg(self, X1, X2, Y1, Y2, sz1=1, sz2=1, S=1, Ntot1=None, Ntot2=None, chol=True):
    ...
    Kuu_chol1 = self.get_Kuu_chol_1(kernel=ker,from_cache=(not chol))
    Kuu_chol2 = self.get_Kuu_chol_2(kernel=ker,from_cache=(not chol))
    #kl_terms is not affected by minibatching so use reduce_sum
    kl_term1 = tf.reduce_sum(self.eval_kl_term_1(mu_z1, Kuu_chol1)) 
    kl_term2 = tf.reduce_sum(self.eval_kl_term_2(mu_z2, Kuu_chol2))
    Mu1 = self.sample_predictive_mean_1(X1, sz=sz1, S=S, kernel=ker, mu_z=mu_z1, Kuu_chol=Kuu_chol1)
    Mu2 = self.sample_predictive_mean_2(X2, sz=sz2, S=S, kernel=ker, mu_z=mu_z2, Kuu_chol=Kuu_chol2)
  ...
  def split_self12(self,self1,self2,L):
    ...
    with tf.name_scope("variational"):
      # variational parameters -rep1
      self_1.delta.assign(self.delta1,read_value=False)#=tf.Variable(self.delta1,name="mean")
      #self_1.delta=tf.Variable(self.delta1,name="mean")
      #self_1.Omega_tril.assign(self.Omega_tril1,read_value=False)
      # variational parameters -rep2
      self_2.delta.assign(self.delta2,read_value=False)#=tf.Variable(self.delta1,name="mean")
    ...
    with tf.name_scope("gp_mean"):
      if self.nonneg:
        ...
        self_1.beta0.assign(self.beta0_1,read_value=False)
        #rep2
        #self2.beta0 = tf.Variable(self.beta0_2,name="intercepts")
        self_2.beta0.assign(self.beta0_2,read_value=False)
      else:
        #rep1
        self_1.beta0.assign(self.beta0_1,read_value=False)
        #rep2
        self_2.beta0.assign(self.beta0_2,read_value=False)
      #rep1
      self_1.beta.assign(self.beta_1,read_value=False)
      #rep2
      self_2.beta.assign(self.beta_2,read_value=False)
    # gp_kernel
    with tf.name_scope("gp_kernel"):
      #self.nugget = tf.Variable(nugget, dtype=dtp, trainable=False, name="nugget")
      #self1.amplitude = tv(self.amplitude1, tfb.Softplus(), dtype=dtp, name="amplitude")
      #self2.amplitude = tv(self.amplitude2, tfb.Softplus(), dtype=dtp, name="amplitude")
      self_1.amplitude.assign(self.amplitude1,read_value=False)
      self_2.amplitude.assign(self.amplitude2,read_value=False)
      #self._ls0 = length_scale #store for .reset() method
      if self.isotropic:
        self_1.length_scale.assign(self.amplitude1,read_value=False)
        self_2.length_scale.assign(self.amplitude2,read_value=False)
      else:
        self_1.length_scale.assign(self.amplitude1,read_value=False)
        self_2.length_scale.assign(self.amplitude2,read_value=False)
    #Loadings weights - shared by rep1 and rep2
    if self.nonneg:
      #rep1
      self_1.W.assign(self.W ,read_value=False) #JxL
      #rep2
      self_2.W.assign(self.W ,read_value=False) #JxL
    else:
      #rep1
      self_1.W.assign(self.W ,read_value=False) #JxL
      #rep2
  ...
  def elbo_avg_self12(self,X1, X2, Y1, Y2, sz1, sz2, self1_0, self2_0, S=1, Ntot=None, chol=True):
    #self1,self2=self.split_self12(self1_0, self2_0,L=4)
    self.W[1]
    return self1.elbo_avg(X1, Y1, sz=sz1, S=S, Ntot=Ntot, chol=chol) 
