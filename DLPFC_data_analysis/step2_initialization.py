...
fit12_=pf_multiSample.ProcessFactorization_fit12(J,L,
  np.concatenate((D1['Z'], D2['Z'],D3['Z']), axis=0),
  nsample=len(list_D__),
  psd_kernel=ker,nonneg=True,lik="poi")


fit12_.init_loadings(np.concatenate((D1['Y'], D2['Y'],D3['Y']), axis=0),
  list_X=[D1['X'], D2['X'],D3['X']],
  list_Z=[D1['Z'],D2['Z'],D3['Z']],
  sz=np.concatenate((D1['sz'], D2['sz'], D3['sz']), axis=0),shrinkage=0.3)
