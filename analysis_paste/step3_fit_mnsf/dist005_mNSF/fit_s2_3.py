...
# inducing points, 90% of total spots for each sample
for ksample in range(0,nsample):
	random.seed(111)
	ninduced=round(list_D[ksample]['X'].shape[0] * 0.9)
	print(ninduced)
	D=list_D[ksample]
	rd_ = random.sample(range(0, D['X'].shape[0]), ninduced)
	list_D[ksample]["Z"]=D['X'][rd_ ,:]
	process_multiSample.save_object(rd_, 'rd_induced_'+str(sample1)+"_"+str(sample1+1)+'_s'+str(ksample+1)+'.pkl') 
