...
class ProcessFactorization(tf.Module):
  def __init__(self, J, L, Z, lik="poi", psd_kernel=tfk.MaternThreeHalves,
    ...
    with tf.name_scope("variational"):
      ...
      #_Omega_tril = ##?still in the memory?
      # _Omega_tril = .01*tf.eye(M,batch_shape=[L],dtype=dtp)
      self.Omega_tril=tv(self._init_Omega_tril(L,M,nugget=nugget), tfb.FillScaleTriL(), dtype=dtp, name="covar_tril") #LxMxM #don't need this in the initialization
    #GP hyperparameters
  ...
  def get_Kuu_chol(self, kernel=None, from_cache=False):
    if not from_cache:
      ...
      print("dim(Kuu_chol)")
      print(Kuu_chol.shape)#(7, 424, 424)
      print(self.Kuu_chol.shape)#(7, 3460, 3460)
  ...
  def sample_latent_GP_funcs(self, X, S=1, kernel=None, mu_z=None, Kuu_chol=None, chol=True):
    ...
    alpha_x = tfl.cholesky_solve(Kuu_chol, kernel.matrix(self.Z, X) ) #LxMxN
    print("dim_delta")
    print(self.delta.shape)
    mu_tilde = mu_x + tfl.matvec(alpha_x, self.delta-self.get_mu_z(), transpose_a=True) #LxN
    ...
    #print("a_t_Omega_tril_before")
    #print(psutil.Process().memory_info().rss / (1024 * 1024 * 1024))
    #a_t_Omega_tril =  #LxNxM
    #print("a_t_Omega_tril_after")
    #print(psutil.Process().memory_info().rss / (1024 * 1024 * 1024))
    aOmega_a = tf.reduce_sum(tf.square(tfl.matmul(alpha_x, self.Omega_tril, transpose_a=True)), axis=2) #LxN
  ...
  def eval_kl_term(self, mu_z, Kuu_chol):
    ...
    """
    print("eval_kl_term_shapes")
    print(self.delta.shape)#(7, 4226)
    print(self.Omega_tril.shape)#(7, 4226, 4226)
  ...
  def elbo_avg(self, X, Y, sz=1, S=1, Ntot=None, chol=True):
    ...
    print("elbo_avg_sample_predictive_mean")
    print(psutil.Process().memory_info().rss / (1024 * 1024 * 1024))
    eloglik = likelihoods.lik_to_distr(self.lik, Mu, self.disp).log_prob(Y)##can be splitted(as Ys are iid across spots?)
    return J*tf.reduce_mean(eloglik) - kl_term/Ntot

  def elbo_avg_component1(self, X, Y, sz=1, S=1, Ntot=None, chol=True):
    """
    Parameters
    ----------
    X : numpy array of spatial coordinates (NxD)
    ...
    #Kuu_chol = 
    #kl_terms is not affected by minibatching so use reduce_sum
    #kl_term = tf.reduce_sum(self.eval_kl_term(mu_z, Kuu_chol)) ker
    Mu = self.sample_predictive_mean(X, sz=sz, S=S, kernel=ker, mu_z=self.get_mu_z(), 
      Kuu_chol=self.get_Kuu_chol(kernel=ker,from_cache=(not chol)))
    print("elbo_avg_sample_predictive_mean")
    print(psutil.Process().memory_info().rss / (1024 * 1024 * 1024))
    eloglik = likelihoods.lik_to_distr(self.lik, Mu, self.disp).log_prob(Y)
    return J*tf.reduce_mean(eloglik)
    #return J*tf.reduce_mean(eloglik)


  def elbo_avg_component2(self, X, Y, sz=1, S=1, Ntot=None, chol=True):
    """
    Parameters
    ----------
    X : numpy array of spatial coordinates (NxD)
    ...
    #Kuu_chol = 
    #kl_terms is not affected by minibatching so use reduce_sum
    kl_term = tf.reduce_sum(self.eval_kl_term(self.get_mu_z(), 
      self.get_Kuu_chol(kernel=self.get_kernel(),
      from_cache=(not chol))))
    #kl_term = tf.reduce_sum(self.eval_kl_term(self.get_mu_z(), self.get_Kuu_chol(kernel=self.get_kernel(),from_cache=(not chol))))
    #Mu = self.sample_predictive_mean(X, sz=sz, S=S, kernel=ker, mu_z=mu_z, Kuu_chol=Kuu_chol)
    print("elbo_avg_sample_predictive_mean")
    print(psutil.Process().memory_info().rss / (1024 * 1024 * 1024))
    #eloglik = likelihoods.lik_to_distr(self.lik, Mu, self.disp).log_prob(Y)
  ...
  def train_step_try(self, D, optimizer, optimizer_k, S=1, Ntot=None, chol=True):
    """
    ...
    with tf.GradientTape(persistent=True) as tape:
      loss1 = -self.elbo_avg_component1(D["X"], D["Y"], sz=D["sz"], S=S, Ntot=Ntot, chol=chol)#?large
      loss2 = -self.elbo_avg_component2(D["X"], D["Y"], sz=D["sz"], S=S, Ntot=Ntot, chol=chol)
    try:
      gradients = tape.gradient(loss, self.trvars_nonkernel)###bottleneck of memory usage
      if chol:
  ...
  def train_step_test(self, D, optimizer, optimizer_k, S=1, Ntot=None, chol=True):
    """
    Executes one training step and returns the loss.
    D is training data: a tensorflow dataset (from slices) of (X,Y,sz)
    This function computes the loss and gradients, and uses the latter to
    ...
    """   
    with tf.GradientTape(persistent=True) as tape:
      loss1 = -self.elbo_avg_component1(D["X"], D["Y"], sz=D["sz"], S=S, Ntot=Ntot, chol=chol)
      loss2 = -self.elbo_avg_component2(D["X"], D["Y"], sz=D["sz"], S=S, Ntot=Ntot, chol=chol)
    try:
      gradients1 = tape.gradient(loss1, self.trvars_nonkernel)###bottleneck of memory usage
      gradients2 = tape.gradient(loss2, self.trvars_nonkernel)###bottleneck of memory usage
      if chol:
