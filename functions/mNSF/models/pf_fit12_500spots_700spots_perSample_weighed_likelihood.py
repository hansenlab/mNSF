  ...
  def __init__(self, J, L, Z, lik="poi", psd_kernel=tfk.MaternThreeHalves,
    ...
    with tf.name_scope("gp_mean"):
      if self.nonneg:
        ...
        self.beta0 = tf.Variable(prior_mu*tf.ones((L+L,1)), dtype=dtp,
                                 name="intercepts")
      else:
        self.beta0 = tf.Variable(tf.zeros((L+L,1)), dtype=dtp, name="intercepts") #L
      self.beta = tf.Variable(tf.zeros((L+L,D)), dtype=dtp, name="slopes") #LxD
...
def smooth_spatial_factors(F,Z,X=None):
  ...
  if X is None: #no spatial coordinates, just use the mean
    ...
    lr1 = LinearRegression().fit(X[0:900,:],F[0:900,:])
    lr2 = LinearRegression().fit(X[900:1800,:],F[900:1800,:])
    ...
    knn1 = KNeighborsRegressor(n_neighbors=nn).fit(X[0:900,:],F[0:900,:])
    knn2 = KNeighborsRegressor(n_neighbors=nn).fit(X[900:1800,:],F[900:1800,:])
    U1 = knn1.predict(Z[0:900,:])
    U2 = knn2.predict(Z[900:1800,:])
...
class ProcessFactorization_self12(tf.Module):
  def __init__(self, self1_0, self2_0, J, L, Z1, Z2, lik="poi", psd_kernel=tfk.MaternThreeHalves, 
    ...
    with tf.name_scope("variational"):
      ...
      self.delta1 = tf.Variable(self1_0.delta.numpy, dtype=dtp, name="mean_1") #LxM
      _Omega_tril1 = self._init_Omega_tril(L=L,M=M1)
      #_Omega_tril1 = self._init_Omega_tril(L,M1,nugget=nugget)
      # _Omega_tril = .01*tf.eye(M,batch_shape=[L],dtype=dtp)
      self.Omega_tril1=tv(self1_0.Omega_tril.numpy, tfb.FillScaleTriL(), dtype=dtp, name="covar_tril_1") #LxMxM
    #variational parameters-rep2
    #with tf.name_scope("variational"):
      self.delta2 = tf.Variable(self2_0.delta.numpy, dtype=dtp, name="mean_2") #LxM
      _Omega_tril2 = self._init_Omega_tril(L,M2)
      #_Omega_tril2 = self._init_Omega_tril(L,M2,nugget=nugget)
      # _Omega_tril = .01*tf.eye(M,batch_shape=[L],dtype=dtp)
      self.Omega_tril2=tv(self2_0.Omega_tril.numpy, tfb.FillScaleTriL(), dtype=dtp, name="covar_tril_2") #LxMxM
    ###GP hyperparameters
    with tf.name_scope("gp_mean"):
      if self.nonneg:
        prior_mu, prior_sigma = misc.lnormal_approx_dirichlet(max(L,1.1))
         #rep1
        self.beta0_1 = tf.Variable(self1_0.beta0.numpy, dtype=dtp,
                                 name="intercepts_1")
         #rep2
        self.beta0_2 = tf.Variable(self2_0.beta0.numpy, dtype=dtp,
                                 name="intercepts_2")
      else:
        #rep1
        self.beta0_1 = tf.Variable(self1_0.beta0.numpy, dtype=dtp, name="intercepts_1") #L
        #rep2
        self.beta0_2 = tf.Variable(self2_0.beta0.numpy, dtype=dtp, name="intercepts_2") #L
      #rep1
      self.beta_1 = tf.Variable(self1_0.beta.numpy, dtype=dtp, name="slopes_1") #LxD
      #rep2
      self.beta_2 = tf.Variable(self2_0.beta.numpy, dtype=dtp, name="slopes_2") #LxD
    # gp_kernel
    with tf.name_scope("gp_kernel"):
      self.nugget = tf.Variable(nugget, dtype=dtp, trainable=False, name="nugget")
      ...
      self._ls0 = length_scale #store for .reset() method
      if self.isotropic:
        self.length_scale1 = tv(self1_0.length_scale.numpy, tfb.Softplus(),
                               dtype=dtp, name="length_scale_1")
        self.length_scale2 = tv(self2_0.length_scale.numpy, tfb.Softplus(),
                               dtype=dtp, name="length_scale_2")
      else:
        #rep1
        self.scale_diag1 = tv(self1_0.scale_diag.numpy,
                             tfb.Softplus(), dtype=dtp, name="scale_diag_1")
        #rep2
        self.scale_diag2 = tv(self2_0.scale_diag.numpy,
                             tfb.Softplus(), dtype=dtp, name="scale_diag_2")
    #Loadings weights - shared by rep1 and rep2
    if self.nonneg:
      self.W = tf.Variable(self1_0.W, dtype=dtp,
                           constraint=misc.make_nonneg, name="loadings") #JxL
    else:
      self.W = tf.Variable(self1_0.W, dtype=dtp, name="loadings") #JxL
    self.psd_kernel = psd_kernel #this is a class, not yet an object
    #likelihood parameters, set defaults
    self._disp0 = disp
    self._init_misc()###>>>
  ...
  def elbo_avg(self, X1, X2, Y1, Y2, sz1=1, sz2=1, S=1, Ntot1=None, Ntot2=None, chol=True):#Ntot1!!,Ntot2!!
    ...
    sum1=sum1/sum12
    sum2=sum2/sum12
    loss_=J1*tf.reduce_mean(eloglik1)*sum1 - kl_term1/500*sum1 + J2*tf.reduce_mean(eloglik2)*sum2 -kl_term2/700*sum2
    return loss_
...
def split_self12(self,self1,self2,L):
  ...
  with tf.name_scope("gp_kernel"):
    ...
    if self.isotropic:
      ...
      self1.length_scale = tv(nself.amplitude1.numpy, name="length_scale")
      self_2.length_scale = tv(nself.amplitude2.numpy, name="length_scale")
    else:
      self_1.length_scale.assign(self.amplitude1.numpy)
...
def split_self12(self,self1,self2,L):
  ...
  with tf.name_scope("gp_kernel"):
    ...
    if self.isotropic:
      #self_1.length_scale.assign(self.amplitude1.numpy)
      #self_1.length_scale.assign(self.amplitude2.numpy)
      self1.length_scale = tv(self.amplitude1.numpy, tfb.Softplus(),dtype=dtp, name="length_scale")                              
      self_2.length_scale = tv(self.amplitude2.numpy, tfb.Softplus(),dtype=dtp, name="length_scale")                               
    else:
      self_1.length_scale.assign(self.amplitude1.numpy)
      self_2.length_scale.assign(self.amplitude2.numpy)
  #Loadings weights - shared by rep1 and rep2
