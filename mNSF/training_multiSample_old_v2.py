...
def train_model_mNSF(self,pickle_path_,
        		list_Dtrain_,list_D_):
	tro_ = ModelTrainer(self,pickle_path=pickle_path_)
	tro_.train_model(list_Dtrain_,list_D_)
  ...
  def _train_model_fixed_lr(self,list_D, list_D__, ckpt_mgr,Dval=None, #Ntr=None,  
    ...
    while (not self.converged) and (self.epoch < num_epochs):
      ...
      #for ksample in range(0,nsample):
        ...
        #assign_para_fromFile_toModel(self.model,ksample)
        with open('fit_tmp'+ str(ksample+1) +'.pkl', 'rb') as inp:
              self.model = pickle.load(inp)   
        print("after loading fit_tmp")
        print(tf.config.experimental.get_memory_info('GPU:0'))
        self.model.Z=list_D__[ksample]["Z"]
        for D in list_D[ksample]: #iterate through each of the batches ####???????!!!!!!!
          print("before training paras")
          print(tf.config.experimental.get_memory_info('GPU:0'))
          epoch_loss.update_state(self.model.train_step( D, self.optimizer, self.optimizer_k,
                                   Ntot=self.model.delta.shape[1], chol=True))     
          print("before saving fit_tmp")
          print(tf.config.experimental.get_memory_info('GPU:0'))
          process_multiSample.save_object(self.model, 'fit_tmp'+str(ksample+1)+'.pkl') 
          print("after saving fit_tmp")
          print(tf.config.experimental.get_memory_info('GPU:0'))
