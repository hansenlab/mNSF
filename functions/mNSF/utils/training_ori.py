...
class ModelTrainer(object): #goal to change this to tf.module?
  ...
  def __init__(self, model, lr=0.01, pickle_path=None, max_to_keep=3, **kwargs):
    ...
    **kwargs are passed to tf.optimizers.[Optimizer] constructor
    """
    self.loss = {"train":np.array([np.nan]), "val":np.array([np.nan])}#value of loss
    self.model = model
    #optimizer for all variables except kernel hyperparams
    #An optimizer is a function or an algorithm that modifies the attributes of 
    # the neural network, such as weights and learning rate. 
    #allowed_kwargs = {
    #'input_dim',
    #'input_shape',
    #'batch_input_shape',
  ...
  def multiply_lr(self,factor): #multiply learning rate by factor
    lr = self.optimizer.learning_rate #lr: learning rate
    lr_old = lr.numpy()
  ...
  def pickle(self,*args):
    ...
    *args passed to update_times method
    """
    p,w = self.update_times(*args)#wtime and ptime
    if self.converged:
  ...
  def _train_model_fixed_lr(self, ckpt_mgr, Dtrain, Ntr, Dval=None, S=3,
    ...
    def train_step(D, chol=True):
      ...
      return self.model.train_step(D, self.optimizer, self.optimizer_k, # see train_step's definition in pf.py
                                   S=S, Ntot=Ntr, chol=chol)  #assume all batches shares the same parameters (? mean?)
    cvg = 0 #increment each time we think it has converged - how many times it has converged
    ...
    while (not self.converged) and (self.epoch < num_epochs):
      ...
      for D in Dtrain: #iterate through each of the batches ####???????!!!!!!!
        print("Xshape")
        print(D["X"].shape)#(347, 2)
        #print(D["Z"].shape)#(347, 2)
      ...
      if not np.isfinite(trl) or trl>self.loss["train"][1]:#check if the loss has diverged
        raise NumericalDivergenceError
      if i%status_freq==0 or i==num_epochs: #if this iteraction is the iteration to check convergence of reaches the max number of epochs
        if Dval:
