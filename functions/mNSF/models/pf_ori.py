...
class ProcessFactorization(tf.Module):
  def __init__(self, J, L, Z, lik="poi", psd_kernel=tfk.MaternThreeHalves,
    ...
    with tf.name_scope("variational"):
      ...
      _Omega_tril = self._init_Omega_tril(L,M,nugget=nugget)##?still in the memory?
      # _Omega_tril = .01*tf.eye(M,batch_shape=[L],dtype=dtp)
      self.Omega_tril=tv(_Omega_tril, tfb.FillScaleTriL(), dtype=dtp, name="covar_tril") #LxMxM #don't need this in the initialization
    #GP hyperparameters
  ...
  def train_step(self, D, optimizer, optimizer_k, S=1, Ntot=None, chol=True):
    ...
    try:
      ...
      print("train_step_finally")
      print(psutil.Process().memory_info().rss / (1024 * 1024 * 1024))
      del tape
      print("train_step_del_tape")
      print(psutil.Process().memory_info().rss / (1024 * 1024 * 1024))
    return loss
