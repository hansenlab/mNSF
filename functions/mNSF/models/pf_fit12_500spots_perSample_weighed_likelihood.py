...
def smooth_spatial_factors(F,Z,X=None):
  ...
  if X is None: #no spatial coordinates, just use the mean
    ...
    lr1 = LinearRegression().fit(X[0:500,:],F[0:500,:])
    lr2 = LinearRegression().fit(X[500:,:],F[500:,:])
    ...
    knn1 = KNeighborsRegressor(n_neighbors=nn).fit(X[0:500,:],F[0:500,:])
    knn2 = KNeighborsRegressor(n_neighbors=nn).fit(X[500:,:],F[500:,:])
    U1 = knn1.predict(Z[0:500,:])
    U2 = knn2.predict(Z[500:,:])
...
class ProcessFactorization_self12(tf.Module):
  ...
  def elbo_avg(self, X1, X2, Y1, Y2, sz1=1, sz2=1, S=1, Ntot1=None, Ntot2=None, chol=True):#Ntot1!!,Ntot2!!
    ...
    sum1=sum1/sum12
    sum2=sum2/sum12
    loss_=J1*tf.reduce_mean(eloglik1)*sum1 - kl_term1/500*sum1 + J2*tf.reduce_mean(eloglik2)*sum2 -kl_term2/500*sum2
    return loss_
